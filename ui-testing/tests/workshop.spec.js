...
test.describe("Search", () => {
  ...
  test("First page of results should contain linkedin.com", async ({
    page,
  }) => {
    let searchPage = new SearchPage(page);
    await searchPage.navigate();
    ...
    for (let index = 0; index < count; index++) {
      const text = await searchResultsPage.getMainLinkAttributeFromResult(
        index + 1
      );
  ...
  test("Search query should be auto-populated in Results page", async ({
    page,
  }) => {
    let searchPage = new SearchPage(page);
    await searchPage.navigate();
